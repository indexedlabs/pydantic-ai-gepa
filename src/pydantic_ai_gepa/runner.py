    ...
    *,
    ...
    Args:
        ...
        max_token_cost: Optional positive run-level US dollar cap, including reflection
            ...
            to ``max_concurrent_evaluations``), so a cheap first case can no
            longer start a full batch on an underestimated reservation. The
            residual overshoot is the amount in-flight rollouts exceed the
            kind's observed high at their admission: at most
            ``min(max_concurrent_evaluations, observations)`` rollouts times
            that excess. The first rollout of a kind runs alone and can
            overshoot by its own cost. Reflection overshoot stays bounded by
